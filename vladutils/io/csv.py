...
class HeaderGetter(ABC):
    ...
    def __init__(self, filename, header_row=0):
        self.filename = filename
        self.header_row = header_row
        self.header = self.get_row(filename, header_row)
        self.get_data()
    ...
    def get_row(self, fp, rw):
        ...
        If header_row == 0, this is easy; just return the 'fieldnames'
        attribute of csv.DictReader. Otherwise, call DictReader.next()
        until we get to the correct row.
        ...
        rw : int
            Row number whose values we want to return as strings.
        """
...
class FilteredCSV(HeaderGetter):
    ...
    def get_data(self):
        self.data = pd.read_csv(
            self.filename, header=self.header_row, usecols=self.header)
...
class FloatCSV(FilteredCSV):
    ...
    def get_data(self):
        return pd.read_csv(
            self.filename, engine='c', dtype=np.float32, usecols=self.header)
